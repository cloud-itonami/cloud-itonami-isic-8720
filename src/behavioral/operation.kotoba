(ns behavioral.operation
  "OperationActor -- one behavioral-care operation = one supervised
  actor run, expressed as a langgraph-clj StateGraph. The advisor
  (BehavioralOps-LLM) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the Behavioral Care Governor
  ...
  One graph run = one behavioral-care operation (intake -> advise ->
  ...
  decision to a human operator (the licensed behavioral-health
  professional). The approver resumes with `{:approval {:status
  :approved}}` (or :rejected). `:treatment-plan/finalize`/`:crisis-
  response/finalize` ALWAYS reach this node when the governor is clean
  -- see `behavioral.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [behavioral.behavioralopsllm :as behavioralopsllm]
...
(defn build
  ...
  `behavioral.store/Store`).
  opts:
    :advisor      -- a `behavioral.behavioralopsllm/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (behavioralopsllm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; BehavioralOps-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (behavioralopsllm/-advise advisor store request)]
            {:proposal p :audit [(behavioralopsllm/trace request p)]})))

      ;; Behavioral Care Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
